...
function canSimplify(data: IDataType) {
...
function getUInt8Buffer(data: IDataType) {
...
function update(data: IDataType) {
  if (!initialized) {
...
function calculate(data: IDataType) {
  if (!canSimplify(data)) {
    init();
    update(data);
    return digest();
...
export function md5(data: IDataType) {
